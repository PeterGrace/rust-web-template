...
use jsonwebtoken::{jwk::{JwkSet, AlgorithmParameters}, decode, DecodingKey, Validation};
use std::any::Any;
...
impl JwksCache {
    ...
    async fn get_or_fetch_jwks(&self, issuer: &str, jwks_uri: &str) -> Result<JwkSet, AuthError> {
        ...
        // Store in cache
        self.keys.write().await.insert(issuer.to_string(), jwks.clone());
            ...
            if auth_str.starts_with("Bearer ") {
                ...
                match decode::<JwtClaims>(
                    ...
                    Err(e) => {
                        ...
                        let app_state = <dyn Any>::downcast_ref::<AppState>(state).ok_or(AuthError::InvalidState)?;

                        // For Google's JWKS (you can add more issuers as needed)
                        let google_jwks = app_state.jwks_cache
                            .get_or_fetch_jwks(
