...
pub async fn get_users(
    ...
    if let Ok(authorized) = user
        .is_authorized::<User>(&state.pool, &AuthorizableType::User(User::default()), &RBAC::Read)
        .await
    {
        if !authorized {
            return Err((
    ...
    return Err((StatusCode::NOT_IMPLEMENTED, AppAPIResponse::message("auth not implemented")));
...
pub async fn get_user(
    ...
    if let Ok(authorized) = user
        .is_authorized::<User>(&state.pool, &AuthorizableType::User(User{id:id, ..User::default()}), &RBAC::Read)
        .await
    {
        if !authorized {
            return Err((
...
pub async fn is_authorized<T: Authorizable>(
    ...
    Ok(T::check_authorization(pool, &id, &self, rbac).await?)
}
}
...
pub async fn update_user(
    ...
    if let Ok(authorized) = user
        .is_authorized::<User>(&state.pool, &AuthorizableType::User(User{ id: id, ..User::default()}), &RBAC::Write)
        .await
    {
        if !authorized {
            return Err((
...
pub async fn delete_user(
    ...
    if let Ok(authorized) = user
        .is_authorized::<User>(&state.pool, &AuthorizableType::User(User{id:id,..User::default()}), &RBAC::Delete)
        .await
    {
        if !authorized {
            return Err((
